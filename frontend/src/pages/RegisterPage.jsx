...
import { User, Mail, Phone, KeyRound, MapPin, UserPlus, ArrowLeft, ShoppingBag } from 'lucide-react';
    ...
    <main className="min-h-screen w-full flex items-center justify-center bg-slate-950 text-slate-100 p-4 md:p-8 relative overflow-hidden bg-gradient-to-tr from-slate-950 via-slate-900 to-slate-950 select-none">
      ...
      <div className="w-full max-w-xl relative z-10">
        <div className="text-center mb-8">
          ...
          <h1 className="mt-4 text-2xl font-extrabold text-white tracking-tight">Đăng ký VieShop</h1>
          <p className="mt-2 text-xs text-slate-400">Tạo tài khoản mới để bắt đầu trải nghiệm mua sắm cao cấp</p>
        </div>
        ...
        <section className="rounded-3xl border border-white/10 bg-white/5 backdrop-blur-md p-8 shadow-2xl relative">
          <form className="grid gap-5 md:grid-cols-2" onSubmit={handleSubmit}>
            <div className="space-y-1.5">
              <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Họ và tên</label>
            ...
            <div className="space-y-1.5 md:col-span-2">
              <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Địa chỉ giao hàng</label>
              <div className="relative">
                <MapPin size={16} className="absolute left-4 top-5 text-slate-500" />
                <textarea
                  className="w-full rounded-xl border border-white/10 bg-white/5 py-3 pl-12 pr-4 text-sm text-white outline-none focus:border-brand-500 focus:bg-white/10 transition"
                  rows="3"
            ...
            {error ? <p className="md:col-span-2 text-xs font-semibold text-red-400">{error}</p> : null}
            
            <button className="hover-shine md:col-span-2 w-full rounded-full bg-brand-500 hover:bg-brand-600 py-3.5 text-center text-xs font-bold text-white shadow-lg shadow-brand-500/20 transition-all duration-200 mt-2 flex items-center justify-center gap-2">
              <UserPlus size={14} />
          ...
          <div className="mt-8 border-t border-white/5 pt-6 text-center text-xs text-slate-450">
            Đã có tài khoản?{' '}
            <Link to="/login" className="font-bold text-brand-400 hover:text-brand-300 transition hover:underline">
              Đăng nhập ngay
            </Link>
          </div>
        </section>
